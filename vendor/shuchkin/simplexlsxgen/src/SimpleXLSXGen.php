...
class SimpleXLSXGen {
    ...
    public function __construct() {
        ...
        $this->sheets = [ ['name' => 'Hoja1', 'rows' => [], 'hyperlinks' => [] ] ];
...
{SHEETS}',
            'xl/worksheets/hoja1.xml' => '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
    xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"
><dimension ref="{REF}"/>{COLS}<sheetData>{ROWS}</sheetData>{HYPERLINKS}</worksheet>',
            'xl/worksheets/_rels/hoja1.xml.rels' => '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
    ...
    protected function _write( $fh ) {
        ...
        foreach ($this->template as $cfilename => $template ) {
            if ( $cfilename === 'xl/_rels/workbook.xml.rels' ) {
                ...
                for ( $i = 0; $i < $cnt_sheets; $i++) {
                    $s .= '<Relationship Id="rId'.($i+2).'" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet"'.
                         ' Target="worksheets/hoja'.($i+1).".xml\"/>\n";
                }
            ...
            elseif ( $cfilename === 'docProps/core.xml' ) {
                ...
                $this->_writeEntry($fh, $cdrec, $cfilename, $template);
                $entries++;
            } elseif ( $cfilename === 'xl/worksheets/hoja1.xml' ) {
                foreach ( $this->sheets as $k => $v ) {
                    $filename = 'xl/worksheets/hoja'.($k+1).'.xml';
                    $xml = $this->_sheetToXML($k, $template);
                    $this->_writeEntry($fh, $cdrec, $filename, $xml );
                    $entries++;
                }
                $xml = null;
            } elseif ( $cfilename === 'xl/worksheets/_rels/hoja1.xml.rels' ) {
                foreach ( $this->sheets as $k => $v ) {
                    if ( count($v['hyperlinks'])) {
                        $RH = [];
                        $filename = 'xl/worksheets/_rels/hoja' . ( $k + 1 ) . '.xml.rels';
                ...
                foreach ( $this->sheets as $k => $v) {
                    $TYPES[] = '<Override PartName="/xl/worksheets/hoja'.($k+1).
                            '.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>';
                    if (count( $v['hyperlinks'])) {
                        $TYPES[] = '<Override PartName="/xl/worksheets/_rels/hoja'.($k+1).'.xml.rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>';
                    }
                }
